...
public:
	...
	void Trace(const char *name) {
		printf("Mempool %s watermark=%d [%dk] current=%d size=%d nAlloc=%d blocks=%d\n", name, _maxAllocs, _maxAllocs
				* ITEM_SIZE / 1024, _currentAllocs, ITEM_SIZE, _nAllocs, _blockPtrs.Size());
	}
...
public:
	...
	void PrintChildElements();

	// Check if child element exists by name.

	bool ChildElementExists(const char *value) const;
...
public:
    ...
    /// Query as a long integer. See IntValue()
	long LongValue() const {
		long i = 0;
		QueryLongValue(&i);
		return i;
	}
	/// Query as an unsigned long integer. See IntValue()
	unsigned UnsignedLongValue() const {
		unsigned long i = 0;
		QueryUnsignedLongValue(&i);
		return i;
	}
    /// Query as an int64 integer. See IntValue()
	int64_t Int64Value() const {
		int64_t i = 0;
		QueryInt64Value(&i);
		return i;
	}
    /// Query as an unsigned int64 integer. Ser IntValue()
	...
	/// See QueryIntValue
	XMLError QueryUnsignedLongValue(unsigned long *value) const;
	/// See QueryIntValue
...
public:
	...
	unsigned int GetUnsignedAttribute(const char *name) const {
		const XMLAttribute *a = FindAttribute(name);
		unsigned int value = 0;
		if (!a) {
			return value;
		}
		a->QueryUnsignedValue(&value);
		return value;
	}
	...
	XMLError QueryLongAttribute(const char *name, long *value) const {
		...
		return a->QueryLongValue(value);
	}

	long GetLongAttribute(const char *name) const {
		const XMLAttribute *a = FindAttribute(name);
		long value = 0;
		if (!a) {
			return value;
		}
		a->QueryLongValue(&value);
		return value;
	}

	/// See QueryIntAttribute()

	XMLError QueryUnsignedLongAttribute(const char *name, unsigned long *value) const {
		const XMLAttribute *a = FindAttribute(name);
		if (!a) {
			return XML_NO_ATTRIBUTE;
		}
		return a->QueryUnsignedLongValue(value);
	}

	unsigned long GetUnsignedLongAttribute(const char *name) const {
		const XMLAttribute *a = FindAttribute(name);
		unsigned long value = 0;
		if (!a) {
			return value;
		}
		a->QueryUnsignedLongValue(&value);
		return value;
	}
	...
	bool GetBoolAttribute(const char *name) const {
		const XMLAttribute *a = FindAttribute(name);
		bool value = false;
		if (!a) {
			return value;
		}
		a->QueryBoolValue(&value);
		return value;
	}
	...
	const char* GetStringAttribute(const char *name) const {
		const XMLAttribute *a = FindAttribute(name);
		if (!a) {
			return "\0";
		}
	...
	bool AttributeExists(const char *name) const {
		const XMLAttribute *a = FindAttribute(name);
		if (!a) {
			return false;
		}
	...
	XMLError QueryAttribute(const char *name, long *value) const {
		return QueryLongAttribute(name, value);
	}

	XMLError QueryAttribute(const char *name, unsigned long *value) const {
		return QueryUnsignedLongAttribute(name, value);
	}
	...
	/// Sets the names attribute to value.
...
enum Whitespace {
	PRESERVE_WHITESPACE, COLLAPSE_WHITESPACE, PEDANTIC_WHITESPACE
};
